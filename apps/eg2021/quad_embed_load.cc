...
int main(int argc, char **argv)
{
    ...
    opts.add_options()("m,max_subdiv", "Max base complex subdivisions.", cxxopts::value<int>()->default_value("0"));

    opts.add_options()("s,smooth_iter", "Apply smoothing post-process based on [Praun2001] for s iterations", cxxopts::value<int>()->default_value("1"));

    opts.add_options()("h,help", "Help.");
    opts.parse_positional({"target"});
    opts.positional_help("[target]");
    ...
    try
    {
        ...
        embedding_path = args["target"].as<std::string>();
        ...
        if (args.count("help") || args.count("target") == 0)
        {
    ...
    // Load embedding
    embedding_path = embedding_path.stem();
    EmbeddingInput input;
    Embedding em(input);
    em.load(embedding_path);

    // Compute integer-grid map
    pm::edge_attribute<int> l_subdivisions;
    if (max_subdiv > 0)
        l_subdivisions = choose_loop_subdivisions(em, edge_length, max_subdiv);
    else
        l_subdivisions = choose_loop_subdivisions(em, edge_length);

    // Compute parameterization
    HalfedgeParam param = parametrize_patches(em, l_subdivisions);

    // Extract quad mesh
    pm::Mesh q;
    pm::face_attribute<pm::face_handle> q_matching_layout_face;
    pm::vertex_attribute<tg::pos3> q_pos = extract_quad_mesh(em, param, q, q_matching_layout_face);

    // Save quad mesh
    const fs::path quad_obj_path = output_dir / (embedding_path.stem().string() + "_" + "quad.obj");
